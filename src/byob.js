/*
    ...
    Copyright (C) 2022 by Jens MÃ¶nig
...
CustomBlockDefinition.prototype.dropDownMenuOf = function (inputName) {
    ...
    if (this.declarations.has(inputName) &&
        ...
        if ((this.declarations.get(inputName)[2].indexOf('Â§_') === 0)) {
...
function CustomHatBlockMorph(definition, isPredicate, isProto) {
    this.init(definition, isPredicate, isProto);
}

CustomHatBlockMorph.prototype.init = function (
    definition,
    isPredicate,
    isProto
) {
    ...
    this.isGlobal = definition ? definition.isGlobal : false;
    this.isPrototype = isProto || false; // optional
    CustomHatBlockMorph.uber.init.call(this, isPredicate, true); // sil.
    if (isProto) {
        this.isTemplate = true;
    }
    this.category = definition.category;
...
CustomHatBlockMorph.prototype.refresh = function (aDefinition) {
    var def = aDefinition || this.definition;
    CustomCommandBlockMorph.prototype.refresh.call(this, aDefinition, true);
    if (!this.isPrototype) {
        this.isPredicate = (def.type === 'predicate');
...
BlockLabelFragment.prototype.hasSpecialMenu = function () {
    return contains(
        /*[
            'Â§_messagesMenu',
            'Â§_messagesReceivedMenu',    //for backward (5.0.0 - 5.0.3) support
            'Â§_objectsMenu',
            'Â§_costumesMenu',
            'Â§_soundsMenu',
            'Â§_getVarNamesDict',
            'Â§_pianoKeyboardMenu',
            'Â§_directionDialMenu'
        ],*/
        [
            '§_messagesMenu',
...
BlockLabelFragment.prototype.hasExtensionMenu = function () {
    return contains(
        Array.from(SnapExtensions.menus.keys()).map(str => 'Â§_ext_' + str),
        this.options
    );
};
...
InputSlotDialogMorph.prototype.specialOptionsMenu = function () {
    ...
    // addSpecialOptions('data types', 'Â§_typesMenu');
...
InputSlotDialogMorph.prototype.extensionOptionsMenu = function () {
    ...
    selectors.forEach(sel => {
        addSpecialOptions(sel.slice(4), 'Â§_ext_' + sel);
    });
    return menu;
};
